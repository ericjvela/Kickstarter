...
###Tree Based Methods
```{r}
newKickStar <- subset(KickStar, select = -c(1,2,3,4,5,6,8,10,12,14,15))
```

```{r}
tree <- rpart(binomState ~ . -c(ID, name, category, main_category, currency, deadline, launched, state, country newDeadline, newLaunched), data = KickTrain)
```
