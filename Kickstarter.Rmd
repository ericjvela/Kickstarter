...
###Find summary statistics when the Kickstarter succeded (1) or failed (0)
```{r}
library(doBy)
summaryBy(goal + pledged + backers + currency ~ binomState, data = KickTest, fun = c(mean,sd))
```
It appears that Kickstarters that had less funding, less pledges, less currency, and less backers failed.
...
###Creating a glm model on the training data and summary
```{r}
glmmodTest <- glm(binomState ~ goal + pledged + backers + currency, data = KickTest, family = binomial)
summary(glmmodTest)
```
...
myFormula <- as.formula(binomState ~ main_category + currency + goal + pledged + backers + country + usd.pledged)
...
library(rpart)
tree <- rpart(state ~ ., data = KickTrain)
```
...
randForPredictTrain <- predict(randFor, newdata = KickTrain, mtry = 3)
randForTrainMSE = mean((AutoTrain$mpg - randForPredictTrain)^2)

randForPredictTest <- predict(randFor, newdata = KickTest, mtry = 3)
