...
ggplot(data = KickStar, aes(backers)) + geom_density(kernel = "gaussian") + facet_wrap(~state)
...
ggplot(data = KickStar, aes(main_category, fill = state)) + geom_bar(position = "dodge") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(title = "Count of Categories by Main Categories", x = "Main Categories", y = "Total Count") 

##ggplot of bar
ggplot(data = KickStar, aes(x = state, y = pledged, fill = state)) + geom_boxplot() + facet_wrap(~currency) +  labs(title = "Boxplots with Amount Pledges by Currecny", x = "State", y = "Total Amount Pledged") 
...
Need more backers to succeed.
Average PLedged is higher with less backer.
